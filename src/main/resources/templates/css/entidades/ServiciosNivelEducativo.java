...
public class ServiciosNivelEducativo extends MetodosDB<NivelEducativo> {
    private static entidades.ServiciosNivelEducativo instancia;

    private ServiciosNivelEducativo(){super(NivelEducativo.class);}

    public static entidades.ServiciosNivelEducativo getInstancia(){
        if(instancia==null){
            instancia = new entidades.ServiciosNivelEducativo();
        }
        return instancia;
    }
    ...
    public boolean crearNiveles(String stringNiveles){
        ...
        if(query.getResultList().size()==0) {
            for (String s : stringNiveles.split(","))
                entidades.ServiciosNivelEducativo.getInstancia().crear(new NivelEducativo(s.trim()));
            return true;
        }
        return false;
    }
